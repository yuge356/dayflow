...
- 未登录用户访问 `/` 时先看到 DayFlow 欢迎页。该页为**浅色扁平 + 液态玻璃**风格：纯色浅灰底（`#f4f5f9`，不使用渐变或装饰背景），内容全部承载在高透玻璃卡片里（细白描边、柔和投影、`backdrop-filter` 毛玻璃）
  - **主视觉：任务树完成动效**。玻璃窗口里横向排布“项目 → 模块 → 任务”，按 `子任务 → 所属模块 → 项目` 推进：叶子任务先被裁剪蒙版从左向右“注入”彩色液体、白色对号沿路径画出、时长切换为 `100%` 且徽标变为“已完成”；随后这条连接线被同一液体点亮汇入上级；两个子任务都完成后模块被填满；最后整个项目节点被填满打勾，停留片刻后循环
  - **液态填充**：完成色不是静止色块——渐变矩形在裁剪区内持续左右漂移（9s），另有一道白色高光横扫（7s），合起来是流动的液体质感。这两段动画用 CSS `transform` 驱动而不是 SMIL：`<animateTransform>` 作用在 `gradientTransform` 上时，一旦标签页被节流就会停在某一帧不再采样
  - 卡片内每一行都有独立基线（`METRICS` 按节点类型写死，不再由卡片高度推算），标签、标题、时长、状态徽标和对号互不重叠
  - 整棵树（节点、文字、连接线、对号）都在一个 `viewBox` 内，因此在任何屏幕宽度下等比缩放
  - **滚动划入 / 收回**：向下滚动时各区块从下方淡入上移，向上滚回时原样退回，两个方向完全对称。实现用滚动位置直接判定（`rect.top < 视口高度 × 0.86`），不用 `IntersectionObserver`——后者只在阈值穿越时回调，要做到对称收回需要成对的 margin，且不便验证
  - **动效是装饰，不承担可读性**：隐藏状态只在 `revealArmed` 打开后生效，而它在首次渲染前就确定（避免内容先显示再淡出的闪烁）；若模板 ref 没有注册则 2 秒后自动放弃隐藏。`prefers-reduced-motion: reduce` 下关闭全部动画与过渡，任务树直接显示为全部完成的终态
