...
- 未登录用户访问 `/` 时先看到 DayFlow 欢迎页。首页讲的是**一条随滚动推进的学习旅程**，强调累积与达成，而不是任务之间的依赖关系：
  ...
  - **左侧汇总**：环形完成度随滚动连续上升（0% → 100%），同时显示"已完成 N / 6 节"和随进度累加的投入小时数。圆环不是一条细线：渐变描边（蓝 → 紫 → 薄荷）+ 投影，外圈一层缓慢呼吸的光晕，**内部是随进度一格格灌满的液体**——两条波形（前后不同速度与透明度）横向漂移形成水面，水位由完成度决定，被圆形蒙版裁在环内。注意只旋转两条弧线让进度从 12 点开始，而不是旋转整个 `svg`：后者会把水面一起转成竖直的
