...
「临时任务」**没有引入新的层级形态**：在今日页随手记下、还没想好归属的任务，会被挂到一个标题为
`临时任务` 的普通 `PROJECT` 下（首次使用时自动创建，按标题复用）。它和项目里的任务共用同一张表、同一
套字段和同一条触发器规则，可以计时、排期、统计；归入别的项目只是一次普通的 `parent_id` 更新。

这样选择的原因是**部署不需要额外迁移**：Vercel 不会自动执行 Alembic，如果「临时任务」依赖放宽层级
触发器的迁移（`0019`），没执行迁移的数据库就会在离线队列里堆积被拒绝的写入。迁移 `0019` 仍然保留
（它只是放宽触发器，已执行的库不受影响），但功能本身不再依赖它。
